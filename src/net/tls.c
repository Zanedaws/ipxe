...
static int tls_set_cipher ( struct tls_connection *tls,
	...
	DBGC( tls, "TLS %p: Pubkey context size: %lu\n", tls, pubkey->ctxsize);
	DBGC( tls, "TLS %p: Cipher context size: %lu\n", tls, cipher->ctxsize);
	DBGC( tls, "TLS %p: Digest context size: %lu\n", tls, digest->ctxsize);
                                         ...
                                         const void *data, size_t len )
{
	...
	{
		DBGC(tls, "TLS %p: Not enough bytes used%d\n", tls, signature_header);
		// no sig
		return -1;
	}
